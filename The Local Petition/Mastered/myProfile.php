<html>
	...
	<head>
		...
		<?php
  			...
  			while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC))
  			{
  				...
  				$query = "SELECT * FROM globalNewsfeedTable WHERE (postID='$currentID' AND userIDFrom='$userID')";
  			...
  			{	
  				...
  				if ($row['hasSupportedGlobal'] == 0)
  				{
  					...
  					$remarksString[] = $newRow['firstName']." ".$newRow['lastName']." SUPPORTS this post.";
  			...
  			function repostBtnClicked($conn, $userID, $postID)
  			{
				...
				$remarksString[] = $newRow['firstName']." ".$newRow['lastName']." REPOSTED this post.";
				$remarksString = implode(',', $remarksString);
  			...
  			function remarkBtnClicked($conn, $userID, $postID, $comment)
  			{
				...
				$remarksString[] = $newRow['firstName']." ".$newRow['lastName']." REMARKED this post - $comment";
				$remarksString = implode(',', $remarksString);
  			...
  			$query = "SELECT firstName,lastName,myLocality,myPhoneNumber,myEmail,myProfilePicture FROM globalUserTable WHERE userID='$userID'";
  			$result = mysqli_query($conn, $query);
  			$row = mysqli_fetch_array($result, MYSQLI_ASSOC);
	...
	<body style="background-color: #E5E5E5;" onload="setFollowing(); setFollower(); makeNewsfeed();">
                    ...
                    <ul class="nav navbar-nav navbar-left">
                        ...
                        <li class="dropdown" id="navBarDropdown">
                            ...
                            <ul class="dropdown-menu" role="menu">
                                ...
                                <li>
                                    <a href="myProfile.php" disabled>My Profile</a>
                                </li>
    	...
    	<script type="text/javascript">
			...
			function makeNewsfeed()
			{
				...
				for (i = 0; i < newsfeedPosts.length; i++)
				{
					...
					hasSupportedGlobal = "<a href='myProfile.php?postIDLiked="+newsfeedPosts[i][0]+"'><button class='btn btn-success viewPostButton'>Support<br>"+newsfeedPosts[i][13]+"</button></a>"

					if (newsfeedPosts[i][2] == 1)
					{
						hasSupportedGlobal = "<a href='myProfile.php?postIDLiked="+newsfeedPosts[i][0]+"'><button class='btn btn-primary viewPostButton'>Unsupport<br>"+newsfeedPosts[i][13]+"</button></a>"
					}
					...
					var postBox ="<div class='viewPost'><div class='col-lg-9 col-md-9 col-sm-9 col-xs-9 viewPostClassHeight'><div class='postExchange'>"+newsfeedPosts[i][3]+"<b> &gt </b> "+newsfeedPosts[i][5]+" "+newsfeedPosts[i][4]+"</div><div class='viewPostTitle'>"+newsfeedPosts[i][7]+"</div><div class='viewPostType'>"+newsfeedPosts[i][6]+"</div>"+forEvent+"<div id='viewPostHover'><p id='viewPostText'>"+smallText+"</p><a href='postView.php?postID="+newsfeedPosts[i][0]+"'>Click here to read more</a></div></div><div class='col-lg-3 col-md-3 col-sm-3 col-xs-3'>"+hasSupportedGlobal+"<button class='btn btn-info viewPostButton' onclick='viewRemarks("+i+");'>Remark<br>"+newsfeedPosts[i][15]+"</button><a href='myProfile.php?postIDShared="+newsfeedPosts[i][0]+"'><button class='btn btn-danger viewPostButton'>Repost<br>"+newsfeedPosts[i][14]+"</button></a></div></div><div id='emptySpaceBetweenPosts'></div>"
