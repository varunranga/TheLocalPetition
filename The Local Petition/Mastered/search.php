<html>
  	...
  	<head>
  		...
  		<?php
            ...
            $whatToQuery = $_GET['search'];

            if ($whatToQuery == 'Petition')
            {
                $query = "SELECT * FROM globalNewsfeedTable WHERE postType='Petition'";
            }
            else
            if ($whatToQuery == 'Event')
            {
                $query = "SELECT * FROM globalNewsfeedTable WHERE postType='Event'";
            }
            else
            if ($whatToQuery == 'Article')
            {
                $query = "SELECT * FROM globalNewsfeedTable WHERE postType='Article'";
            }
            else
            {
                $query = "SELECT * FROM globalNewsfeedTable WHERE postTitle LIKE '%$whatToQuery%' UNION SELECT * FROM globalNewsfeedTable WHERE postText LIKE '%$whatToQuery%'";
            }
  			...
  			function remarkBtnClicked($conn, $userID, $postID, $comment)
  			{
				...
				$newRow['firstName']." ".$newRow['lastName']." REMARKED this post - $comment";
    	...
    	<title>
    		Search for - &quot<?php echo $whatToQuery; ?>&quot
    	</title>
